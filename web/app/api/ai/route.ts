...
export async function POST(request: Request) {
  const guard = await guardApiRequest(request, { bucket: "ai-generate", limit: 12, maxBodyBytes: 64_000 });
  ...
  try {
    ...
    return Response.json(await generateText({ system: SYSTEM, user, credential, provider: body.provider, model: body.model, maxTokens: mode === "summary" ? 500 : 900 }), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
