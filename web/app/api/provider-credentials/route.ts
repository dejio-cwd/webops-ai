...
export async function GET(request: Request) {
  ...
  const response = await fetch(`${value.url}/rest/v1/provider_credentials?select=id,organization_id,provider,display_name,key_hint,base_url,model_allowlist,scopes,status,created_at,rotated_at,revoked_at&organization_id=eq.${encodeURIComponent(organizationId)}&order=created_at.desc`, { headers: headers(value.key), cache: "no-store" });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!row?.id) return Response.json({ error: "Credential creation returned no record." }, { status: 502 });
    await audit(organizationId, actor.id, "credential.created", row.id);
...
export async function PATCH(request: Request) {
  ...
  if (!body.credentialId || !isUuid(body.credentialId) || !body.apiKey || !await canManage(actor, organizationId)) return Response.json({ error: "Owner or admin access and a new secret are required." }, { status: 403 });
  const value = configuration(); if (!value) return Response.json({ error: "Credential vault is not configured." }, { status: 503 });
  try {
    const now = new Date().toISOString(); const response = await fetch(`${value.url}/rest/v1/provider_credentials?id=eq.${encodeURIComponent(body.credentialId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", headers: headers(value.key, "return=representation"), body: JSON.stringify({ encrypted_secret: encryptSecret(body.apiKey), key_hint: body.apiKey.slice(-4), status: "active", rotated_at: now, revoked_at: null }), cache: "no-store" });
    ...
    if (!updated) return Response.json({ error: "Credential not found in this workspace." }, { status: 404 });
    await audit(organizationId, actor.id, "credential.rotated", updated.id); return Response.json({ ok: true, keyHint: body.apiKey.slice(-4), rotatedAt: now });
...
export async function DELETE(request: Request) {
  ...
  try { body = await request.json(); } catch { return Response.json({ error: "Invalid request." }, { status: 400 }); }
  const organizationId = body.organizationId || "";
  if (!body.credentialId || !isUuid(body.credentialId) || !await canManage(actor, organizationId)) return Response.json({ error: "Owner or admin access required." }, { status: 403 });
  const value = configuration(); if (!value) return Response.json({ error: "Credential vault is not configured." }, { status: 503 });
  const now = new Date().toISOString(); const response = await fetch(`${value.url}/rest/v1/provider_credentials?id=eq.${encodeURIComponent(body.credentialId)}&organization_id=eq.${encodeURIComponent(organizationId)}`, { method: "PATCH", headers: headers(value.key, "return=representation"), body: JSON.stringify({ status: "revoked", revoked_at: now }), cache: "no-store" });
  if (!response.ok) return Response.json({ error: "Unable to revoke credential." }, { status: 400 });
  const updated = (await response.json() as Array<{ id: string }>)[0];
  if (!updated) return Response.json({ error: "Credential not found in this workspace." }, { status: 404 });
  await audit(organizationId, actor.id, "credential.revoked", updated.id); return Response.json({ ok: true, revokedAt: now });
}
