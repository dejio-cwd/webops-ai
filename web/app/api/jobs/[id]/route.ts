...
import { dispatchJob } from "@/lib/pipeline/launch";
...
export async function GET(request:Request,context:Context){
 ...
 try{const {id}=await context.params;const job=isUuid(id)?await authorizeJob(id,actor.id):null;if(!job)return Response.json({error:"Audit not found."},{status:404});return Response.json({job,progress:await getProgress(id)},{headers:{"Cache-Control":"no-store"}});}catch{return Response.json({error:"Unable to load audit progress."},{status:503});}
}
export async function PATCH(request:Request,context:Context){
 ...
 if(action!=="pause" && ["QUEUED","RUNNING","CANCELLING"].includes(status))await dispatchJob(id);
 return Response.json({status});}catch{return Response.json({error:"Unable to update this audit. Its saved evidence is retained."},{status:503});}
}
