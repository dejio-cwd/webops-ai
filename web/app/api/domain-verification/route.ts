...
async function projectAccess(actor: AuthenticatedActor, projectId: string, mutate: boolean) {
  ...
  const projectResponse = await fetch(`${value.url}/rest/v1/projects?select=id,owner_id,organization_id,domain,verified_at&id=eq.${encodeURIComponent(projectId)}&limit=1`, { headers: headers(value.key), cache: "no-store" });
  const projects = projectResponse.ok ? await projectResponse.json() as Project[] : [];
  const project = projects[0]; if (!project) return null;
  if (project.owner_id === actor.id) return project;
  const memberResponse = await fetch(`${value.url}/rest/v1/organization_members?select=role&organization_id=eq.${project.organization_id}&user_id=eq.${actor.id}&limit=1`, { headers: headers(value.key), cache: "no-store" });
  const members = memberResponse.ok ? await memberResponse.json() as Array<{ role: string }> : [];
  const allowed = mutate ? ["owner", "admin", "developer"] : ["owner", "admin", "analyst", "developer", "viewer", "billing"];
  return allowed.includes(members[0]?.role || "") ? project : null;
}
async function candidates(project: Project, method: Verification["method"]) {
  if (method === "dns") {
    const records = await Promise.allSettled([dns.resolveTxt(`_webops.${project.domain}`), dns.resolveTxt(project.domain)]);
  ...
  return method === "file" ? [text.trim()] : (text.match(/webops-ai-verification=[a-f0-9]{48}/gi) || []);
}

export async function GET(request: Request) {
  const actor = await guardApiRequest(request, { bucket: "domain-read", limit: 60, requireAuth: true });
  if (isGuardResponse(actor)) return actor;
  if (!actor) return Response.json({ error: "Authentication required." }, { status: 401 });
  const projectId = new URL(request.url).searchParams.get("projectId") || "";
  const project = await projectAccess(actor, projectId, false);
  if (!project) return Response.json({ error: "Project access denied." }, { status: 403 });
  const value = config(); if (!value) return Response.json({ error: "Verification service is not configured." }, { status: 503 });
  const response = await fetch(`${value.url}/rest/v1/domain_verifications?select=id,project_id,method,status,verified_at,created_at&project_id=eq.${project.id}&order=created_at.desc&limit=10`, { headers: headers(value.key), cache: "no-store" });
  const verifications = response.ok ? await response.json() : [];
  return Response.json({ project: { id: project.id, domain: project.domain, verifiedAt: project.verified_at }, verifications }, { headers: { "Cache-Control": "no-store" } });
}

export async function POST(request: Request) {
  const actor = await guardApiRequest(request, { bucket: "domain-write", limit: 12, windowMs: 300_000, maxBodyBytes: 8_000, requireAuth: true });
  if (isGuardResponse(actor)) return actor;
  if (!actor) return Response.json({ error: "Authentication required." }, { status: 401 });
  let body: { projectId?: string; action?: "start" | "verify"; method?: Verification["method"] };
  try { body = await request.json(); } catch { return Response.json({ error: "Invalid request." }, { status: 400 }); }
  const method = body.method || "dns"; if (!["dns", "file", "meta"].includes(method)) return Response.json({ error: "Invalid verification method." }, { status: 400 });
  const project = await projectAccess(actor, body.projectId || "", true);
  if (!project) return Response.json({ error: "Developer, admin, or owner access required." }, { status: 403 });
  const value = config(); if (!value) return Response.json({ error: "Verification service is not configured." }, { status: 503 });
