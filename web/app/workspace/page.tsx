...
type Member = { user_id: string; role: string; email: string | null; created_at: string }; type Invitation = { id: string; email: string; role: string; expires_at: string; accepted_at: string | null; revoked_at: string | null; created_at: string };
...
export default function WorkspacePage() {
  ...
  const [organization, setOrganization] = useState<Organization | null>(null); const [organizations, setOrganizations] = useState<Organization[]>([]); const [projects, setProjects] = useState<Project[]>([]); const [activeProjectId, setActiveProjectId] = useState("");
  const [members, setMembers] = useState<Member[]>([]); const [invitations, setInvitations] = useState<Invitation[]>([]); const [email, setEmail] = useState(""); const [role, setRole] = useState("viewer"); const [invitationLink, setInvitationLink] = useState("");
  ...
  const refreshWorkspace = useCallback(async () => {
    ...
    const availableOrganizations = (organizationData.memberships || []).map((item) => item.organizations).filter((item): item is Organization => Boolean(item));
    const firstOrganization = availableOrganizations[0];
    const nextProjects = projectData.projects || [];
    const selectedOrganization = availableOrganizations.find((item) => item.id === organization?.id) || firstOrganization;
    const visibleProjects = selectedOrganization ? nextProjects.filter((project) => project.organization_id === selectedOrganization.id) : nextProjects;
    setUserEmail(sessionData.user?.email || ""); setOrganizations(availableOrganizations); setOrganization(selectedOrganization || null); setProjects(nextProjects);
    setActiveProjectId((current) => current && visibleProjects.some((project) => project.id === current) ? current : visibleProjects[0]?.id || "");
  }, [router]);
  ...
  const refreshInvitations = useCallback(async (organizationId: string) => {
    const response = await apiFetch(`/api/organizations/invitations?organizationId=${encodeURIComponent(organizationId)}`);
    if (response.ok) { const data = await response.json() as { invitations?: Invitation[] }; setInvitations(data.invitations || []); }
  }, []);

  useEffect(() => { refreshWorkspace().catch(() => setError("Unable to load the workspace.")).finally(() => setLoading(false)); }, [refreshWorkspace]);
  useEffect(() => { if (organization) { refreshMembers(organization.id).catch(() => null); refreshInvitations(organization.id).catch(() => null); } }, [organization, refreshMembers, refreshInvitations]);
  ...
  const visibleProjects = organization ? projects.filter((project) => project.organization_id === organization.id) : projects;
  ...
  function openModule(name: string) { if (name === "AI Studio") { router.push("/ai-studio"); return; } setActive(name); setPaletteOpen(false); clearMessages(); }
  ...
  async function createInvitation() {
    if (!organization || !email) return; setBusy(true); clearMessages(); setInvitationLink("");
    const response = await apiFetch("/api/organizations/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId: organization.id, email, role }) });
    const data = await response.json() as { acceptPath?: string; error?: string };
    if (!response.ok) setError(data.error || "Unable to create invitation."); else { setInvitationLink(`${window.location.origin}${data.acceptPath || ""}`); setNotice("Invitation created. Copy the link and send it securely to the invited person."); }
    setBusy(false);
  }
  async function revokeInvitation(invitationId: string) {
    if (!organization || !window.confirm("Revoke this invitation? The link will stop working.")) return;
    setBusy(true); clearMessages();
    const response = await apiFetch("/api/organizations/invitations", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ organizationId: organization.id, invitationId }) });
    const data = await response.json() as { error?: string };
    if (!response.ok) setError(data.error || "Unable to revoke invitation."); else { setNotice("Invitation revoked."); await refreshInvitations(organization.id); }
    setBusy(false);
  }
  ...
  async function updateEnvironment(projectId: string, environment: string) {
    ...
    setBusy(true); clearMessages();
    const response = await apiFetch("/api/projects", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, organizationId: organization.id, environment }) });
    const data = await response.json() as { error?: string };
    if (!response.ok) setError(data.error || "Unable to update project environment."); else { setNotice(`Environment changed to ${environment}.`); await refreshWorkspace(); }
    setBusy(false);
  }
  async function deleteProject(projectId: string) {
    if (!organization || !window.confirm("Delete this project and its verification records? This cannot be undone.")) return;
    setBusy(true); clearMessages();
    const response = await apiFetch("/api/projects", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectId, organizationId: organization.id }) });
    const data = await response.json() as { error?: string };
    if (!response.ok) setError(data.error || "Unable to delete project."); else { setNotice("Project deleted."); await refreshWorkspace(); }
    setBusy(false);
  }
  ...
  const projectsPanel = <section className={styles.console}><div className={styles.consoleHead}><div><span>PROJECT GOVERNANCE</span><h2>Projects and environments</h2><p>Keep crawl targets, environments, and ownership verification inside a governed tenant boundary.</p></div><button className={styles.primary} onClick={() => router.push("/onboarding")}>Add project</button></div>{visibleProjects.length ? <><div className={styles.projectPicker}>{visibleProjects.map((project) => <button key={project.id} className={activeProject?.id === project.id ? styles.projectActive : ""} onClick={() => setActiveProjectId(project.id)}><b>{project.name}</b><span>{project.domain}</span><small>{project.environment} · {project.verified_at ? "verified" : "verification pending"}</small></button>)}</div>{activeProject && <div className={styles.verifyBox}><div><span>DOMAIN OWNERSHIP</span><h3>{activeProject.domain}</h3><p>Verification unlocks governed fixes, monitoring, and higher-trust automation.</p></div><div className={styles.inlineForm}><select value={activeProject.environment} onChange={(event) => updateEnvironment(activeProject.id, event.target.value)} disabled={busy}><option value="production">Production</option><option value="staging">Staging</option><option value="development">Development</option></select><select value={verificationMethod} onChange={(event) => setVerificationMethod(event.target.value)}><option value="dns">DNS TXT</option><option value="file">Well-known file</option><option value="meta">Meta tag</option></select><button onClick={startVerification} disabled={busy}>Create challenge</button><button className={styles.primary} onClick={checkVerification} disabled={busy}>Check verification</button><button onClick={() => deleteProject(activeProject.id)} disabled={busy}>Delete project</button></div>{instructions && <pre className={styles.instructions}>{JSON.stringify(instructions, null, 2)}</pre>}</div>}</> : <div className={styles.emptyState}><h3>No projects yet</h3><p>Create the first governed website project to begin collecting evidence.</p><button className={styles.primary} onClick={() => router.push("/onboarding")}>Create project</button></div>}</section>;

  const teamPanel = <section className={styles.console}><div className={styles.consoleHead}><div><span>ROLE-BASED ACCESS</span><h2>Team and permissions</h2><p>Owners and admins can add existing WebOps AI accounts and assign least-privilege roles.</p></div></div>{organization ? <><div className={styles.inlineForm}><input type="email" placeholder="teammate@company.com" value={email} onChange={(event) => setEmail(event.target.value)} /><select value={role} onChange={(event) => setRole(event.target.value)}>{roles.map((name) => <option key={name}>{name}</option>)}</select><button className={styles.primary} onClick={addMember} disabled={busy || !email}>Add member</button><button onClick={createInvitation} disabled={busy || !email}>Create invitation</button></div>{invitationLink && <div className={styles.instructions}><b>Invitation link</b><br />{invitationLink}<br /><button onClick={() => navigator.clipboard?.writeText(invitationLink)}>Copy link</button></div>}<div className={styles.memberList}>{members.map((member) => <article key={member.user_id}><div><b>{member.email || member.user_id}</b><small>Added {new Date(member.created_at).toLocaleDateString()}</small></div><select value={member.role} onChange={(event) => changeRole(member.user_id, event.target.value)} disabled={busy}>{roles.map((name) => <option key={name}>{name}</option>)}</select><button onClick={() => removeMember(member.user_id)} disabled={busy}>Remove</button></article>)}</div>{invitations.length > 0 && <div className={styles.memberList}><h3>Pending invitations</h3>{invitations.map((invitation) => <article key={invitation.id}><div><b>{invitation.email}</b><small>{invitation.role} · expires {new Date(invitation.expires_at).toLocaleDateString()}</small></div><button onClick={() => revokeInvitation(invitation.id)} disabled={busy || Boolean(invitation.revoked_at || invitation.accepted_at)}>{invitation.revoked_at ? "Revoked" : invitation.accepted_at ? "Accepted" : "Revoke"}</button></article>)}</div>}</> : <div className={styles.emptyState}><p>Create a workspace before managing access.</p></div>}</section>;
