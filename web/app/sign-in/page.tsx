...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("confirmed") === "1") {
      setMode("login");
      setMessage("Email confirmed. Sign in to continue to your workspace.");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (mode === "signup" && data.confirmationRequired) {
        ...
        setMessage("If this address can be registered, a confirmation email has been sent. Confirm it, then sign in here.");
  ...
  return <main className={styles.page}><section className={styles.card}>
    ...
    <p className={styles.muted}>{mode === "login" ? "Sign in to continue to protected audits, projects, and governed AI connections." : "Create your secure account. We never reveal whether an address is already registered."}</p>
    ...
    <button className={styles.switcher} onClick={switchMode}>{mode === "login" ? "New to WebOps AI? Create an account" : "Already have an account? Sign in"}</button>
