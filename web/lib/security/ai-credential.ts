...
export type CredentialInput = { credentialId?: string; provider?: string; apiKey?: string; baseUrl?: string; model?: string };
export type CredentialRequest = CredentialInput & { credential?: CredentialInput };
...
export async function credentialFromRequest(actor: AuthenticatedActor | null, body: CredentialRequest): Promise<GovernedCredential | undefined> {
  const storedId = body.credentialId || body.credential?.credentialId;
  if (storedId) {
    const stored = await resolveCredentialForActor(actor, storedId) as GovernedCredential;
    return enforceCredentialPolicy(stored, body.model || body.credential?.model, undefined);
  }
...
export function enforceCredentialPolicy(credential: GovernedCredential, requestedModel: unknown, scope: CredentialScope | undefined): GovernedCredential {
  const scopes = credential.scopes || [];
  if (scope && scopes.length && !scopes.includes(scope) && !(scope === "test" && scopes.includes("models"))) throw new Error("This credential is not permitted for the requested AI operation.");
