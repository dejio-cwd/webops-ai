...
function rejectOversizeBody(request: Request, maxBodyBytes: number): Response | null {
  const length = Number(request.headers.get("content-length") || 0);
  return Number.isFinite(length) && length > maxBodyBytes ? Response.json({ error: "Request body is too large." }, { status: 413 }) : null;
}
function rateLimit(request: Request, options: GuardOptions): Response | null {
  ...
  return state.count <= limit ? null : Response.json({ error: "Rate limit exceeded. Try again later." }, { status: 429, headers: { "Retry-After": String(Math.ceil((state.resetAt - now) / 1000)) } });
}
function checkOrigin(request: Request): Response | null {
  const origin = request.headers.get("origin");
  return !origin || origin === new URL(request.url).origin ? null : Response.json({ error: "Cross-origin request blocked." }, { status: 403 });
}
async function authenticate(request: Request): Promise<AuthenticatedActor | Response> {
  const header = request.headers.get("authorization") || "";
  const accessToken = header.startsWith("Bearer ") ? header.slice(7).trim() : cookie(request, "webops_access");
...
export async function guardApiRequest(request: Request, options: GuardOptions): Promise<AuthenticatedActor | null | Response> {
  ...
  const limitError = rateLimit(request, options); if (limitError) return limitError;
